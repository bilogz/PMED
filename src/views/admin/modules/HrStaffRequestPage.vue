...
const requestedRole = ref<PmedRoleType>('medical_officer');
const requestedCount = ref(1);
const requestNotes = ref('');
const requestedBy = ref('PMED Admin');
const toast = reactive({ open: false, text: '', color: 'info' as 'success' | 'info' | 'warning' | 'error' });
let pollInterval: ReturnType<typeof setInterval> | null = null;
let searchDebounce: ReturnType<typeof setTimeout> | null = null;

const cards = computed(() => [
  { title: 'Active HR Roster', value: status.value?.totals.activeRoster ?? 0, subtitle: 'Active medical staff', icon: 'mdi-account-check-outline', color: '#1565C0' },
  { title: 'Working Roster', value: status.value?.totals.workingRoster ?? 0, subtitle: 'Currently on duty', icon: 'mdi-stethoscope', color: '#2E7D32' },
...
function showToast(text: string, color: typeof toast.color = 'info'): void {
  toast.text = text; toast.color = color; toast.open = true;
}
...
function openDialog(): void {
  ...
  requestedRole.value = 'medical_officer';
...
<template>
  <div class="hr-request-page">
    <v-card class="hero-card" variant="outlined">
      <v-card-text class="d-flex justify-space-between align-center flex-wrap ga-3">
        <div>
          ...
          <h1 class="text-h5 font-weight-black mb-1">Request Medical Staff from HR</h1>
    ...
    <v-dialog v-model="staffDialog" max-width="680">
      <v-card>
        <v-card-item><v-card-title>Request Additional Medical Staff from HR</v-card-title></v-card-item>
        <v-card-text>
          <v-row>
            <v-col cols="12" md="6">
