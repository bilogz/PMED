...
const DEPT_NAME = 'PMED';
export type PmedRoleType = HrStaffIntegrationRoleCode;
...
function resolveSupabaseUrl(): string {
  return trimSlash((import.meta.env.VITE_SUPABASE_URL as string | undefined)?.trim() ?? '');
}
...
async function sbCount(table: string, filter: string): Promise<number> {
  ...
  const res = await fetch(`${resolveSupabaseUrl()}/rest/v1/${table}?select=id${q}`, {
    method: 'HEAD',
...
async function sbGetPaged<T>(path: string): Promise<{ data: T[]; count: number }> {
  assertConfig();
  const res = await fetch(`${resolveSupabaseUrl()}/rest/v1/${path}`, {
    headers: { ...authHeaders(), 'Prefer': 'count=exact' }
  });
  if (!res.ok) throw new Error((await res.text()) || `Request failed (${res.status})`);
  const m = (res.headers.get('Content-Range') ?? '').match(/\/(\d+)$/);
...
type RawRow = {
  id: number; request_reference: string; staff_id: number;
  request_status: string; request_notes: string | null;
...
/** Scope requests to this department’s rows in HR (same Supabase project as HR app). */
const REQ_DEPT_FILTER = `hr_staff_directory.department_name=eq.${encodeURIComponent(DEPT_NAME)}`;

async function sbCountHrRequests(status: string): Promise<number> {
  assertConfig();
  const q = `request_status=eq.${encodeURIComponent(status)}&${REQ_DEPT_FILTER}`;
  const res = await fetch(`${resolveSupabaseUrl()}/rest/v1/hr_staff_requests?select=id&${q}`, {
    method: 'HEAD',
    headers: { ...authHeaders(), 'Prefer': 'count=exact' }
  });
  const m = (res.headers.get('Content-Range') ?? '').match(/\/(\d+)$/);
...
export async function fetchHrStaffRequestStatus(): Promise<HrStaffRequestStatus> {
  ...
  const [active, working, pending, approved, recent] = await Promise.all([
    sbCount('hr_staff_directory', `${roleFilter}&employment_status=eq.active&department_name=eq.${encodeURIComponent(DEPT_NAME)}`),
    sbCount('hr_staff_directory', `${roleFilter}&employment_status=eq.working&department_name=eq.${encodeURIComponent(DEPT_NAME)}`),
    sbCountHrRequests('pending'),
    sbCountHrRequests('approved'),
    sbGet<RawRow>(`hr_staff_requests?${EMBED}&${REQ_DEPT_FILTER}&order=created_at.desc&limit=5`)
  ]);
  return {
...
export async function fetchHrStaffRequests(params: {
  ...
  const parts = [`${EMBED}`, REQ_DEPT_FILTER, `order=created_at.desc`, `limit=${perPage}`, `offset=${offset}`];
...
export async function updateHrStaffRequestStatus(payload: {
  id: number;
  requestStatus: HrStaffRequestRow['request_status'];
  decidedBy?: string;
}): Promise<void> {
