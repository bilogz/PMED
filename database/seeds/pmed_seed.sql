-- PMED-only seed data (PostgreSQL/Supabase)
...
WITH desired_profile AS (
  SELECT
    ...
    COALESCE(
      (SELECT flow_order FROM department_flow_profiles WHERE department_key = 'pmed'),
      (SELECT CASE WHEN EXISTS (SELECT 1 FROM department_flow_profiles WHERE flow_order = 2) THEN COALESCE(MAX(flow_order), 0) + 1 ELSE 2 END FROM department_flow_profiles)
    ) AS flow_order,
    COALESCE(
      (SELECT clearance_stage_order FROM department_flow_profiles WHERE department_key = 'pmed'),
      (SELECT CASE WHEN EXISTS (SELECT 1 FROM department_flow_profiles WHERE clearance_stage_order = 2) THEN COALESCE(MAX(clearance_stage_order), 0) + 1 ELSE 2 END FROM department_flow_profiles)
    ) AS clearance_stage_order,
    '["hr"]'::jsonb AS receives,
    '["clinic"]'::jsonb AS sends,
    'Pre-medical evaluation compliance.'::text AS notes
)
